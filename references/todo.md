...
统一 `bin/byteworker preflight` 已完成 Todo `check`，不要在每个 session 再单独运行。
`TODO_REMINDERS` notice 的 `data.items` 最多带 3 项、`data.total` 给出总数；没有命中时
preflight 完全无输出。只有 Todo 主动增删改或排障时才直接调用 `bin/byteworker todo ...`。
