...
1. **确定性候选召回**:先按机器协议运行
   `bin/byteworker kb-query search --kb "<知识库目录>" --query "<查询>" --limit 12 --graph-depth 1 --max-nodes 30`。
...
2. **结构化大 raw 召回(按场景必做)** —— 用户询问 Meego / Base / 风神中的具体需求、记录/报表 ID、
   ...
   `bin/byteworker kb-query source-record --kb "<知识库目录>" --source-type meego --record-id "<稳定 ID>"`
   或
   `bin/byteworker kb-query source-record --kb "<知识库目录>" --source-type meego --title "<需求标题或用户表述>"`。
