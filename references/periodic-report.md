...
长流程状态输出:执行 `daily` / `weekly` 时先告诉用户会先跑定期摄取、再召回事实源、最后写报告和本地回滚点。每完成下列一个阶段都发一行短状态;任一阶段超过约 30-60 秒时发 heartbeat,说明仍在处理哪个阶段和已处理的来源 / 节点 / 报告数量。
...
普通 `daily` 默认不扫描用户全部 IM。若用户明确说「日报包含 IM」「分析今天聊天里最重要的事」「最近一天 IM 重点」,则在「召回事实源」后加跑 `references/im-inbox-summary.md`:
