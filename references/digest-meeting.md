...
2. 从日程的描述 / 附件里**提取绑定的文档链接**;并尝试定位这场会的妙记(`lark-minutes` 按会议名 / 时间查)。
...
- **妙记 `feishu_minutes`(event 主干)** —— 用 `lark-minutes` 取纪要 + AI 产物(总结 / 待办 / 章节)+ 逐字稿。它提供:真实讨论、与会人、结论、待办、`decision`,以及「参与方立场分析」的**逐字证据源**。event 的「议程与讨论 / 结论 / 待办 / 参与方立场分析」主要据它来写。
