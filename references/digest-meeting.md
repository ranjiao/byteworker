...
唯一的无人值守确认例外是 `references/report-calendar-meetings.md`：用户已明确授权日报 / 周报对
报告周期内 `self_rsvp_status=accept` 的日程处理直接会议产物，因此可不逐场追问。该例外只覆盖
日程直接绑定的纪要 / 录制、会中共享文档和日程描述 / 附件里的直接文档；不得按标题搜索其它
资料、递归摄取依赖或注册 routine 来源。产物缺失 / 无权限按报告覆盖规则记录，不能静默扩大身份。
