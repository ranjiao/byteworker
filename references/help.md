...
后台信息助手   自动检查与提醒 —— 默认关闭，用户明确确认后才启用
  ...
  → 自动健康检查只处理确定性低风险修复；重要问题仍请你决策
  → 说“分析今天/昨天飞书 IM”时，使用一次性 foreground process；不会隐式打开后台
  → 说“看看待关注事项 / 为什么认为这条重要”可复查结果；独立 inbox 命令已移除
  → 会增加网络、模型和存储开销；若要按时运行，机器需保持开机、唤醒、联网
  → 机器休眠或关机期间不运行，恢复后只能补跑
  → 后台助手关闭或故障不影响 digest/search/update 等已有能力
...
能力发现   不用先学命令，根据当前知识库推荐下一步
  "byteworker 还能帮我做什么?" / "根据我的知识库推荐下一步" / "关闭功能建议"
  → 按用户目标展示能力、直接可说的例句、当前使用状态和必要配置
  → 成功任务后最多补充一条相关建议；全局限频，可拒绝、延后或完全关闭
  → 推荐只读本地里程碑，不为了提示访问飞书、日历、网络或发起授权
...
文档:DESIGN.md(存储 schema)· docs/development/plans/backlog.md(延后功能)
