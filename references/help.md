...
更新 skill 不是子命令 —— 说"更新 skill""检查更新"可立即触发自动更新检查(默认每周静默自动检查一次,从 GitHub 拉取最新 skill 内容;无需 GitHub 账号/SSH key)。代码确实更新后会自动运行 doctor、修复确定性低成本问题；严重错误请求你决策，warning/info 只给一行摘要供你选择忽略或立即处理。
