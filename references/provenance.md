...
- 风神:`aeolus:report:<report_id>`;保存
  `region + app_id + dashboard_id + sheet_id + report_id + dataset_id + effective_filters`。
...
3. plan 的 `provenance` 不可省略。`digest-plan/v2` 和 `digest-batch-plan/v2` 的 anchors 只由
   SourceBundle 提供，plan 顶层 provenance 只允许 `enrichment`，不得复制 anchors。每个 node
   必须显式给 `evidence` 数组，新节点至少一条；evidence 逐条给 `id + anchor_id`，跨 raw 时再给
   `raw_id`。历史 v1 plan 才在 plan provenance 中携带 anchors。
