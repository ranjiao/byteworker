...
1. **分类** —— 判定 `source_type`:`feishu_doc` / `feishu_minutes` / `feishu_meeting` / `feishu_chat` / `local_md`。
2. **摄取原文**:
...
digest     摄取 —— 把资料存进知识库
  /byteworker digest <飞书文档/妙记 URL | 会议 | 群 | 本地 md>
