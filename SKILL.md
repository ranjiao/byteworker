...
- **无需 GitHub 账号/SSH key**:仓库是 public repo,脚本会自动使用 HTTPS 拉取;若你当前 origin 是 SSH(`git@github.com`) 但环境无 SSH key,脚本会自动 fallback 到 HTTPS 并把 origin URL 持久切到 HTTPS。
...
4. **冲突检测** —— 先确认 INDEX 一致(见「写入规范」);按标题/人名/项目名在 INDEX 找可能涉及的已有节点,读取候选,语义比对是否与新输入矛盾。**有冲突 → 高亮矛盾点,等用户裁决,不静默覆盖。**
...
`feishu_minutes` / `feishu_meeting` 单独摄取无额外细则(但若它属于一场带投屏文档的会议 → 走上面「会议簇」行);`local_md` 直接读取本地文件 —— 按上面主干执行即可。
...
日报文件 = 知识库数据目录下的 `reports/daily/<YYYY-MM-DD>.md`。它是某一天的工作总结快照,不是知识节点;模板在 skill 目录 `templates/report-daily.md`。

执行细则见 `references/periodic-report.md`。核心流程:
1. **先自动跑定期摄取** —— 不等到期提醒,直接按 `references/digest-routine.md` 复查 INDEX「定期摄取清单」中的滚动文档 / 群聊等来源,消化新增内容;完成后写 `.last-routine-digest`。
2. **确定日期范围** —— 默认当天(本地时区);用户带 `YYYY-MM-DD` 则生成指定日期。
3. **召回材料** —— 读取当日 journal、新增/更新 raw、当日新建/更新节点,并沿相关节点 links 做一跳补充;结合 `context.md` 只读透镜,重点关注使用者本人、团队、主管方向和长期关注项目。
4. **筛选重要事项** —— 优先纳入决策、指标显著变化、事故/风险、项目状态变化、与你/团队直接相关的关键交互;排除纯流水、重复和无留存价值的杂讯。着重分析context.md中有说明需要重点关注的项目。
5. **写日报** —— 确保 `reports/daily/` 存在;复制 `templates/report-daily.md` 结构生成/覆盖当天日报;条目按事件发生时间倒序排列,每条带来源节点或 raw_id;无信息的章节写"暂无"而不是编造。用词简短精炼，避免使用大段文字。
6. **回显摘要** —— 把日报的本日重点、风险 / 下步和文件路径告诉用户。

## weekly — 周报

**触发**:子命令 `weekly`;或自然语言 —— "生成本周周报""更新周报""这周工作总结"。

周报文件 = 知识库数据目录下的 `reports/weekly/<YYYY>-W<WW>.md`。它是一个自然周的工作总结快照,不是知识节点;模板在 skill 目录 `templates/report-weekly.md`。

执行细则见 `references/periodic-report.md`。核心流程:
1. **先自动跑定期摄取** —— 同 `daily`,确保滚动周报、群聊等定期来源先被增量消化。
2. **确定周范围** —— 默认当前 ISO 周(周一 00:00 到当前时刻,本地时区);用户说"上周"则取上一 ISO 周完整周。
3. **召回材料** —— 读取范围内 journal、raw、event/decision 和更新过的实体节点,再沿 links 补充相关 project/person/org/area;必要时参考范围内 daily 报告,但以节点和 raw 为事实源。
4. **归纳主题** —— 按团队 / 项目 / 主题聚合,突出一周内的决策、进展、风险、阻塞、跨团队协作和下周关注点;同一事件不要在多个章节重复堆叠。着重分析context.md中有说明需要重点关注的项目。
5. **写周报** —— 确保 `reports/weekly/` 存在;复制 `templates/report-weekly.md` 结构生成/覆盖本周周报;章节内条目按事件发生时间倒序,每条带来源节点或 raw_id。用词简短精炼，避免使用大段文字。
6. **回显摘要** —— 把周报的本周重点、风险 / 下步和文件路径告诉用户。
...
- **时间格式**:严格按 DESIGN.md §2.1。写入任何 raw frontmatter、knowledge 节点、INDEX、journal、dashboard、reports 前,先把可结构化时间规范化:日期写 `YYYY-MM-DD`,人读时间写 `YYYY-MM-DD HH:MM`,机器边界 / 群聊高水位写 `YYYY-MM-DDTHH:MM:SS+08:00`,ISO 周写 `YYYY-Www`。不要在 skill 生成内容中写 `20260520`、`5-21`、`05/21` 等裸格式;这些只允许保留在 raw 原文正文里。
- **时间顺序**:节点 body 中凡是带日期 / 时间的条目,统一按**事件发生时间倒序**排列(最新发生的在前),而不是按写入时间随手追加。典型章节:`person` 的「协作历史与关键交互」、`project` 的「关联文档与会议 / 关键进展 / 思路与视角 / 历史」、`org` 的「协作历史」、`decision` 的「历史」。新增条目时插入到正确时间位置;日期不明的条目放在该章节末尾并标注时间不明。
...
3. 脚本只动节点文件、不碰 git 与 journal —— 按「写入规范」由你在数据目录 `git add -A && git commit` 并追加一行 journal 收尾;脚本退出码 3 表示仍有悬空链接待处理。
