---
name: byteworker
description: 个人飞书工作知识库。把飞书文档、会议妙记、会议、群聊、本地 md 摄取并消化成结构化实体图笔记(人员/项目/主题领域/组织/事件/决策),支持对话式查询、更新、新鲜度扫描和会议前简报。当用户要把飞书文档/妙记 URL/会议/群聊/本地 md 存入知识库、消化某份资料或某个群的讨论、查询"关于X我知道什么"或"我们关于Y定过什么"、查项目进度、更新某条知识、检查知识库是否过期陈旧、需要会议前简报、或想了解这个 skill 怎么用(help)时使用。
---
...
2. **摄取原文**:
   ...
   - `feishu_chat` → 用 `lark-im` 摄取群聊:`+chat-search` 按群名定位 `chat_id` → `+chat-messages-list` 按时间窗拉取消息(分页拉全)。详见下方「群聊摄取补充」。
   - `local_md` → 直接 Read。
   失败按下方「错误处理」中止。
...
- **拉取**:`lark-cli im +chat-search --query "<群名>"` 定位 `chat_id` → `+chat-messages-list` 按时间窗拉全量消息(**注意分页拉全,别漏**)。
