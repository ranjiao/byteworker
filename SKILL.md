---
name: byteworker
description: 个人飞书工作知识库。把飞书文档、会议妙记、会议、群聊、本地 md 摄取并消化成结构化实体图笔记(人员/项目/主题领域/组织/事件/决策),支持对话式查询、更新、新鲜度扫描和会议前简报。当用户要把飞书文档/妙记 URL/会议/群聊/本地 md 存入知识库、消化某份资料或某个群的讨论、查询"关于X我知道什么"或"我们关于Y定过什么"、查项目进度、更新某条知识、检查知识库是否过期陈旧、需要会议前简报、查看工作看板(dashboard)、或想了解这个 skill 怎么用(help)时使用。
---
...
**触发**:用户给出飞书文档/妙记 URL、会议、群、或本地 md 路径,说"存入知识库""消化这个""记一下""把 XX 群最近的讨论存进来"。
...
- **人员 @ 提及解析**:`lark-doc` 返回的 `<cite type="user">` 是裸 `open_id`(不像群聊会自动解析姓名)。digest 前用 `lark-contact` 把 open_id 反查成姓名 —— 用 `+get-user --user-id <open_id> --as user` 逐个查(`+search-user` 批量查需 `contact:user:search` scope,通常未授权);再建/更新 `person` 节点。
...
1. 摄取 —— 把资料存进知识库
   你说:"把这个文档存进知识库 <飞书URL>" / "消化下这个妙记 <URL>" / "记一下这次会议"
        / "把『XX群』最近一周的关键讨论存进来"
   结果:拉取原文(文档/妙记/会议/群聊)→ 消化成 人员/项目/主题领域/组织/事件/决策 节点 → 入库
...
6. 工作看板 —— 看当下该关注什么
   你说:"看板" / "我要长期关注 X" / "提醒我关注 Y"
   结果:长期关注项(自动拉最新状态)+ 需关注事项 + 今日进展
...
**触发**:`看板`、`dashboard`、"今天进展"、"我要长期关注 X"、"提醒我关注 Y"。
...
2. 刷新派生内容:
   ...
   - ⚠️ 需要关注:跑一次轻量扫描(同能力四:陈旧节点、未裁决冲突)填派生项;手动提醒项原样保留。
...
- **回滚点**:每次摄取/更新/扫描完成后,在知识库数据目录执行 `git add -A && git commit`(该目录自身的本地 git,**永不 push**),使每一步可回滚。
