...
2. **图遍历(检索承重墙,必做)** —— 命中 → 定向 Read 节点 → **必沿 `links` 遍历相关节点**,而不止于 INDEX 关键词命中。关系型问题(「X 涉及哪些人」「Y 项目相关哪些决策」「谁参与了 Z」)尤其依赖此步,只看 INDEX 命中会漏掉一跳之外的关联。综合遍历到的节点回答。**答案必须附 `sources` 出处。**
3. **置信度(必报)**:
...
1. 扫 `knowledge/` 下全部 7 类节点的 frontmatter。
2. 按 DESIGN.md §6 的分节格式重新生成整个 `INDEX.md`;「待消化」表 = 扫 `raw_data/` 中 `digest_status: pending` 的文件。
