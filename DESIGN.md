...
**`project`(实体,广义专项/事项)**
```markdown
## 关联文档与会议   <!-- 该项目被讨论/提及的主要文档/会议/群聊(标题+日期+链接);digest 时持续追加去重 -->
## 目标
## 关键策略
## 关键进展         <!-- 带日期,新进展在前;含里程碑、关键决策、状态变化 -->
## 问题            <!-- 当前待解决的问题/阻塞 -->
## 风险            <!-- 潜在风险 -->
## 成员 / 相关方     <!-- person 链接 -->
## 思路与视角        <!-- 各方对本项目的主观思路/想法/打法/意图;第一方陈述,带日期带作者,标【主张】/【意图】,见 §4.6 -->
## 历史             <!-- 目标/策略被推翻时旧值移入,标来源+日期 -->
```
...
3. **创建或更新实体节点**:输入实质涉及的 person/project/org/area —— 不存在则建,已存在则更新。
   **项目节点尤其要**:把本次输入(文档/会议/群聊)追加进「关联文档与会议」,并刷新
   目标 / 关键策略 / 关键进展 / 问题 / 风险。
...
**增量语义**:同一群多次增量摄取 = 按窗口产生一串 `event`(各自定型、不回改);实体节点
(`project`/`person`/`org`/`area`)跨窗口走实体消解、持续累积更新,不新建重复。

**外部读物(`web`)的扇出**:一篇文章 → **1 个 `reading` 节点**(来源/核心观点/可借鉴点),
不产 event/decision,一般也不动工作实体节点 —— 它与工作的关联靠日后 `links` 长出来。

**会议簇的扇出**:一场飞书会议常散成多个物件 —— 1 个日历日程 + 主讲人投屏的 N 个文档 +
会上产出的 1 份妙记(`feishu_minutes`)。三类物件描述同一场会,digest **当成一个整体**:
只产 **1 个 `event`**(妙记为主干、文档为材料、日历日程提供标题/时间/参会人元信息),
N 个 `decision` 从「妙记 + 文档」并集抽取去重。投屏文档按性质分流 —— 纯会议材料并入
event,实质枢纽文档(PRD/规划/设计)独立 digest 并与 event 双向 `link`。**绝不**把同一场
会的文档和妙记拆成互不相连的 event。详见 SKILL「digest」→ `references/digest-meeting.md`。
...
【主张】/【意图】标记的条目(新条目在前)。须区分第一方陈述与从发言推断,标对出处。
...
`event` 节点除字面结论外,需对关键参与方做立场分析(并同步沉淀进 `person` 节点的
「立场 / 利益 / 动机」章节)。规则:

- **三个维度**:① 立场 / 倾向 —— 对所讨论决策的位置;② 利益 / 动机 —— ta 在为谁、
  为什么优化(代表哪方、核心诉求);③ 对决策的态度 —— 支持 / 反对 / 中立 / 有条件支持。
- **必须基于证据**:每条结论可追溯到 ta 的具体发言或行为,关键处引用原话或注明出处。
- **区分观察与推断**:【观察】= ta 明确说了 / 做了;【推断】= 基于证据的合理推测。
- **不臆测**:证据不足时写「证据有限」或略过,**不做无证据的发散猜测**;
  宁可少写、写得保守,也不编造动机。
...
`project` / `area` 节点设「思路与视角」章节,承载**第一方、主观、被主动输入**的观点 ——
使用者本人的工作思路、主管的想法/方向、合作同事的判断。它与 §4.5「参与方立场分析」互补:
§4.5 是从会议发言**推断**出来的(observed);本章节多为**直接陈述**的(stated)。

- **章节形态 = 带日期、带作者、只追加的日志**(同「关键进展」),不就地改写。
  当前思路 = 最新一条;旧条目保留为可见的演进轨迹,过期太多可滚入「历史」。
- **条目格式**:`- 【主张|意图】<作者:节点 id 或姓名> · <YYYY-MM-DD> —— <内容>`,新条目在前。
  - 作者尽量写成 `person-` 节点 id(自动连边);无对应节点则写姓名。
  - 无明确作者时默认 = 知识库主人(使用者本人)。
- **出处标记**:【主张】= 某人主张/认为如此;【意图】= 某人想做/打算做。
  与 §4.5 的【观察】/【推断】并列 —— 第一方陈述用【主张】/【意图】,从发言推断仍用【推断】。
- **绝不硬化为事实**:这是最易带偏的一层。synthesize/答复时,【主张】/【意图】永远呈现为
  「某人的判断/意图」,不得当成客观结论;与客观信息冲突时主动提示。
- **更老化更快**:思路比 decision 过期快 —— 答复时优先用最新条目,引用旧条目时
  提示「这是 N 周前的思路,可能已变」。

> 与 `context.md` 的分工:本章节是**挂在具体 project/area 上**的观点;`context.md`(§10)是
> **跨主题**的工作底色。digest 时,针对具体项目的思路落到节点本章节,跨主题的留给使用者维护 context.md。
